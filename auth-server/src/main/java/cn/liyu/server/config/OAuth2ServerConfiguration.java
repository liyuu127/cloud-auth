...
public class OAuth2ServerConfiguration extends AuthorizationServerConfigurerAdapter {
    ...
    /**
     * 我们配置了使用数据库来维护客户端信息。
     * 对于实际的应用我们一般都会用数据库信息，还会建立一套工作流来允许客户端自己申请ClientID，实现OAuth客户端接入的审批。
     * @param clients
     * @throws Exception
     */
    @Override
    ...
    /**
     * 首先，打开了验证Token的访问权限。
     * 然后，允许ClientSecret明文方式保存，并且可以通过表单提交（而不仅仅是Basic Auth方式提交)。
     * @param security
     * @throws Exception
     */
    @Override
    ...
    /**
     *
     * 1. 配置我们的令牌存放方式为JWT方式，而不是内存、数据库或Redis方式。JWT保存Token虽然易于使用但是不是那么安全，一般用于内部，且需要走HTTPS并配置比较短的失效时间。
